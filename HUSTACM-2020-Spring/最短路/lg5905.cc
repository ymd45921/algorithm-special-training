...
bool spfa_nega_ring(int st, int n)
{
    ...
    bitset<N> inq; queue<info_t> hot; vector<int> cnt(n + 1);
...
signed main()
{
    ...
    for (int i = 1; i <= n; ++ i)
        FWS::addEdge(0, i, 0);
    bool nega = spfa_nega_ring(0, n + 1);
    if (nega) {puts("-1"); return 0;}
    for (int i = 1; i <= n; ++ i)
        FWS::forEach(i, [](edge &e){e.w += pe[e.u] - pe[e.v];});
    for (int i = 1; i <= n; ++ i)
    {
        dijkstra(i, n);
        lll ans = 0;
        for (int j = 1; j <= n; ++ j)
            if (dis[j] >= inf) ans += 1e9 * (lll)j;
            else ans += (lll)j * (dis[j] - pe[i] + pe[j]);
        println(ans);
    }
    return 0;
}