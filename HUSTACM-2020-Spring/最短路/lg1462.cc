...
void dijkstra(int st, int n, int restrict)
{
...
signed main()
{
    ...
    int fmi = max(f[1], f[n]);
    int ll = fmi, rr = fma, ans = fma + 1;
    while (ll <= rr)
    {
        int mid = (ll + rr) / 2;
        dijkstra(1, n, mid);
        if (dis[n] < b) ans = mid, rr = mid - 1;
        else ll = mid + 1;
    }
    if (ans > fma) puts("AFK");
    else println(ans);
